...
export default class ConditionList extends Component {
    ...
    findAllConditions(currentPage) {
        ...
        axios.get("http://localhost:8082/api/conditions?pageNumber="+currentPage+"&pageSize="+this.state.conditionsPerPage+"&sortBy=id&sortDir="+sortDir)
    ...
    deleteCondition = (conditionId) => {
        axios.delete("http://localhost:8082/api/conditions/"+conditionId)
            .then(response => {
                if(response.data != null) {
                    this.setState({"show":true});
                    setTimeout(() => this.setState({"show":false}), 3000);
                    this.setState({
                        conditions: this.state.conditions.filter(condition => condition.id !== conditionId)
                    });
                } else {
                    this.setState({"show":false});
                }
    ...
    lastPage = () => {
        let condition = Math.ceil(this.state.totalElements / this.state.conditionsPerPage);
    ...
    nextPage = () => {
        if(this.state.currentPage < Math.ceil(this.state.totalElements / this.state.conditionsPerPage)) {
            if(this.state.search) {
    ...
    searchData = (currentPage) => {
        currentPage -= 1;
        axios.get("http://localhost:8082/api/conditions/search/"+this.state.search+"?page="+currentPage+"&size="+this.state.conditionsPerPage)
            .then(response => response.data)
            .then((data) => {
                this.setState({
                    conditions: data.content,
        ...
        return (
            <div>
                <div style={{"display":this.state.show ? "block" : "none"}}>
                    <MyToast show = {this.state.show} message = {"condition supprimé avec succès."} type = {"danger"}/>
                </div>
                              ...
                              <tbody>
                                {
                                    ...
                                    <tr key={condition.id}>
                                        ...
                                        <td>
                                            <ButtonGroup>
                                                <Link to={"editcondition/"+condition.id} className="btn btn-sm btn-outline-primary"><FontAwesomeIcon icon={faEdit} /></Link>{' '}
                                                <Button size="sm" variant="outline-danger" onClick={this.deleteCondition.bind(this, condition.id)}><FontAwesomeIcon icon={faTrash} /></Button>
                                            </ButtonGroup>
                                        </td>
                                    </tr>
                                    ))
